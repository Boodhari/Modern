...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("SELECT full_name, phone FROM visitors WHERE id = ?");
    $stmt->bind_param("i", $visitor_id);
    $stmt->execute();
    $res = $stmt->get_result();
    $visitor = $res->fetch_assoc();

    $insert = $conn->prepare("INSERT INTO appointments (visitor_id, patient_name, phone, appointment_date, reason) VALUES (?, ?, ?, ?, ?)");
    $insert->bind_param("issss", $visitor_id, $visitor['full_name'], $visitor['phone'], $date, $reason);
    $insert->execute();
